...
function inserter_functions.set_arm_positions(inserter, event, res)
    ...
    if (not event.shift and event.button == defines.mouse_button_type.left) or (event.shift and event.button == defines.mouse_button_type.right) then
        --Drop
        inserter.drop_position = {res.base.x+x, res.base.y+y}

        if (event.control) then
            if (event.shift) then
                if (event.button == defines.mouse_button_type.left) then
                    -- Switch drop and pickup if on pickup
                end
            else
        ...
        if sprite_button.sprite == "" then
            sprite_button.sprite = "drop"
        elseif sprite_button.sprite == "combo" then
            sprite_button.sprite = "pickup"
        elseif sprite_button.sprite == "pickup" then
            sprite_button.sprite = "combo"
        end
    elseif (not event.shift and event.button == defines.mouse_button_type.right) or (event.shift and event.button == defines.mouse_button_type.left) then
        --pickup
        inserter.pickup_position = {res.base.x+x, res.base.y+y}

        if (event.control) then
            if (event.shift) then
                if (event.button == defines.mouse_button_type.right) then
                    -- Switch drop and pickup if on drop
                end
            else
                if (event.button == defines.mouse_button_type.left) then
                    -- Switch drop and pickup if on drop
        ...
        elseif sprite_button.sprite == "combo" then
            sprite_button.sprite = "drop"
        elseif sprite_button.sprite == "drop" then
            sprite_button.sprite = "combo"
        end
    end
...
function inserter_functions.inserter_default_range(inserter)
    ...
    biggest.range = math.max(biggest.x, biggest.y) - collision_box_total

    return biggest.range
end
...
--Cleaned
function inserter_functions.old_set_arm_positions(inserter, positions)
